...
I'm not going for super-efficient, minimum layers here, I just want to get it working. Here's the completed, working Dockerfile. We'll come back to the ENTRYPOINT and CMD lines at the end.
...
CMD [ "-p", "tony", "-u", "twitter_user", "-s", "ORADB12C", "-m", "ut011340", "-n", "1525", "-w", "oracle" ]
```

You need to ensure the instant client file is in the same directory as the Dockerfile as well as the stream.py file.

Now for stream.py I wanted to add some parameter. I wanted to be able to set the at the very least the terms we'd be watching twitter for but if I was adding parameter anyway it made sense to take the database username, password, SID, host and port too. I did hard-code the API application keys and I've removed them here so you'll need to add your own if you re-use this code. Here's the final python code.
